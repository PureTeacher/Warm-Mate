    ...
    <view class="container">
      ...
      <view class="form-section" v-if="currentStep === 2">
        ...
        <view class="resend-section" v-if="resendCountdown > 0">
          <text class="resend-text">{{ resendCountdown }}秒后可重新发送</text>
        </view>
        <view class="resend-section" v-else>
          <text class="resend-link" @click="resendCode">重新发送验证码</text>
        </view>
      ...
      <view class="form-section" v-if="currentStep === 3">
        ...
        <text class="tip-text" v-if="!passwordError && newPassword">{{ passwordTip }}</text>
...
export default {
  ...
  computed: {
    ...
    isCodeValid() {
      return this.verificationCode && this.verificationCode.length === 6 && !this.codeError;
    },
    isPasswordValid() {
      return (
        this.newPassword &&
  ...
  methods: {
    ...
    async sendVerificationCode() {
      ...
      try {
        ...
        const response = await uni.$u.http.post(url, {
          phone: this.phone
        }, {
        ...
        if (response && response.code === 200) {
          ...
          if (response.data && response.data.verificationCode) {
            uni.$u.toast(`验证码: ${response.data.verificationCode}`, 3000);
          }
        } else {
          let errorMsg = response?.message || '发送失败，请重试';
          
          // 检查是否是频率限制错误
          if (errorMsg && (errorMsg.includes('frequency') || errorMsg.includes('频繁'))) {
        ...
        if (error && error.data) {
          ...
          if (msg && (msg.includes('frequency') || msg.includes('频繁'))) {
            errorMsg = '请求过于频繁，请稍候再试（建议60秒后重试）';
            this.startResendCountdown();
          } else {
    ...
    validatePassword() {
      ...
      const hasValid = /^[a-zA-Z0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>/?]+$/.test(this.newPassword);
      if (!hasValid) {
